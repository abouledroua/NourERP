...
export async function createTuitionPayment(req, res) {
  ...
  if (
    ...
    return res
      ...
      .json({
        success: false,
        message: "بيانات الوصل المالي غير مكتملة / Incomplete payment details",
      });
  }
  ...
  try {
    ...
    if (discount_type === "PERCENTAGE") {
      effectiveDiscount =
        (Number(amount_due) * Number(discount_value || 0)) / 100;
    ...
    `,
      [
        ...
        payment_method || "CASH",
        status,
        covered_months ? JSON.stringify(covered_months) : null,
        account_id || null,
        req.user?.id || null,
        notes || null,
      ],
    ...
    await logAudit(
      ...
      "payments",
      result.insertId,
      { receipt_number, amount_paid, remaining_debt },
      req.ip,
    );
  ...
  try {
    ...
    if (!payment) {
      return res
        ...
        .json({
          success: false,
          message: "وصل السداد غير موجود / Receipt not found",
        });
    }
...
export async function voidTransaction(req, res) {
  ...
  try {
    if (category === "TUITION") {
      ...
      await executeTransaction(async (conn) => {
        ...
        const items = await conn.query(
          "SELECT product_id, quantity FROM product_sale_items WHERE sale_id = ?",
          [id],
        );
      ...
      return res
        ...
        .json({
          success: false,
          message: "صنف المعاملة غير صالح / Invalid transaction category",
        });
    }
    ...
    res.json({
      success: true,
      message:
        "تم إلغاء/إبطال المعاملة بنجاح واسترجاع المخزون إذا وجد / Transaction voided",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
export async function listFeeTypes(req, res) {
  try {
    const types = await query(
      "SELECT * FROM fee_types WHERE is_active = TRUE ORDER BY id ASC",
    );
...
export async function createCashTransaction(req, res) {
  ...
  if (!transaction_type || !category || !amount || !description) {
    return res
      ...
      .json({
        success: false,
        message: "بيانات العملية المالية غير مكتملة / Required fields missing",
      });
  }
  ...
  try {
    ...
    const [countRow] = await query(
      "SELECT COUNT(id) AS count FROM cash_transactions",
    );
    ...
    await logAudit(
      ...
      "cash_transactions",
      result.insertId,
      { voucher_number, transaction_type, amount },
      req.ip,
    );
    res
      .status(201)
      .json({
        success: true,
        message: "تم تقييد العملية في سجل الخزينة / Cash entry recorded",
        voucher_number,
      });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
export async function createAccount(req, res) {
  ...
  if (!name)
    return res
      ...
      .json({
        success: false,
        message: "اسم الحساب مطلوب / Account name required",
      });
  try {
    if (is_default) {
    ...
    const result = await query(
      "INSERT INTO financial_accounts (name, currency, is_default, balance) VALUES (?, ?, ?, ?)",
      [name, currency || "DZD", is_default || false, balance || 0],
    );
    res.json({
      success: true,
...
export async function updateAccount(req, res) {
  ...
  try {
    ...
    if (cols.length > 0) {
      vals.push(id);
      await query(
        `UPDATE financial_accounts SET ${cols.join(", ")} WHERE id = ?`,
        vals,
      );
    }
    res.json({
      success: true,
      message: "تم تحديث الحساب بنجاح / Account updated",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
export async function deleteAccount(req, res) {
  ...
  try {
    ...
    res.json({
      success: true,
      message: "تم حذف الحساب بنجاح / Account deleted",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
export async function transferFunds(req, res) {
  const { from_account_id, to_account_id, amount, notes, transfer_date } =
    req.body;
  if (!from_account_id || !to_account_id || !amount || amount <= 0) {
    return res
      ...
      .json({
        success: false,
        message: "بيانات التحويل غير صالحة / Invalid transfer details",
      });
  }
  try {
    await executeTransaction(async (conn) => {
      ...
      const [fromRes] = await conn.query(
        "UPDATE financial_accounts SET balance = balance - ? WHERE id = ? AND balance >= ?",
        [amount, from_account_id, amount],
      );
      if (fromRes.affectedRows === 0) {
        throw new Error(
          "رصيد غير كافي أو حساب غير صالح / Insufficient funds or invalid account",
        );
      }
      // Add to receiver
      await conn.query(
      ...
      // Record transfer
      await conn.query(
        "INSERT INTO account_transfers (from_account_id, to_account_id, amount, notes, transfer_date) VALUES (?, ?, ?, ?, ?)",
        [
          from_account_id,
          to_account_id,
          amount,
    ...
    res.json({
      success: true,
      message: "تم التحويل بنجاح / Transfer successful",
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
