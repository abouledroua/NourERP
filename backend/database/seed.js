...
export async function seedDatabase() {
  ...
  const tracks = [
    {
      code: "PRE_SCHOOL",
      ar: "التعليم التحضيري والروضة",
      en: "Early Childhood & Preschool",
      fr: "Enseignement Préscolaire",
      desc: "تنمية المهارات السلوكية والحركية واللغوية للطفولة المبكرة",
    },
    {
      code: "K12_PRIMARY",
      ar: "التعليم الابتدائي",
      en: "Primary Elementary School",
      fr: "Enseignement Primaire",
      desc: "المرحلة الابتدائية من السنة الأولى إلى الخامسة",
    },
    {
      code: "K12_MIDDLE",
      ar: "التعليم المتوسط",
      en: "Middle School / Junior High",
      fr: "Enseignement Moyen",
      desc: "المرحلة المتوسطة من الأولى إلى الرابعة متوسط",
    },
    {
      code: "K12_HIGH",
      ar: "التعليم الثانوي",
      ...
      desc: "المرحلة الثانوية - جذوع مشتركة وشعب تخصصية",
    },
    {
      code: "ACADEMIC_TUTORING",
      ar: "دروس الدعم والتقوية الأكاديمية",
      en: "Academic Tutoring & Remedial",
      fr: "Soutien Scolaire et Rattrapage",
      desc: "حصص مسائية وأسبوعية مكثفة للمراجعة والتحضير للامتحانات الرسمية",
    },
  ];
  ...
  const subjects = [
    ...
    },
    {
      ...
      en: "Physics & Technology",
      fr: "Physique et Technologie",
      track: trackMap["K12_MIDDLE"],
      coeff: 2.0,
      color: "#f59e0b",
    },
    {
      code: "PRE-ACTIV",
      ar: "الأنشطة الحركية والإدراكية",
      en: "Motor & Cognitive Activities",
      fr: "Activités Motrices et Cognitives",
      track: trackMap["PRE_SCHOOL"],
      coeff: 1.0,
      color: "#ec4899",
    },
    {
      code: "TUTOR-BAC",
      ar: "مراجعة الرياضيات المركزة (بكالوريا)",
      en: "Advanced Math Remedial (BAC)",
      fr: "Mathématiques Approfondies (BAC)",
      track: trackMap["ACADEMIC_TUTORING"],
      coeff: 1.0,
      color: "#6366f1",
    },
  ];
  ...
  if (existingYears.length === 0 && existingClasses[0].count === 0) {
  ...
  const [classRows] = await pool.query(
    `SELECT id, academic_track_id, name FROM classes`,
  );
  ...
  await pool.query(
    `
    INSERT INTO students (matricule, national_id, first_name_ar, last_name_ar, first_name_en, last_name_en, gender, birth_date, birth_place, blood_group, current_class_id, academic_track_id, enrollment_date, status, phone, email, address)
    VALUES
      ('STU-2026-0001', '1202016254', 'إياد', 'حليمي', 'Iyad', 'Halimi', 'MALE', '2020-04-12', 'الجزائر العاصمة', 'O+', ?, ?, '2025-09-01', 'ACTIVE', '0551234567', 'halimi.mohamed@gmail.com', 'حي المستقبل، عمارة 12'),
      ('STU-2026-0002', '1201725364', 'ريان', 'بن سالم', 'Rayan', 'Ben Salem', 'MALE', '2017-08-22', 'بومرداس', 'A+', ?, ?, '2025-09-01', 'ACTIVE', '0662345678', 'bensalem.kamel@yahoo.fr', 'شارع الشهداء رقم 45'),
      ('STU-2026-0003', '1201736452', 'سارة', 'عمراوي', 'Sara', 'Amraoui', 'FEMALE', '2017-02-14', 'البليدة', 'B+', ?, ?, '2025-09-01', 'ACTIVE', '0773456789', 'amraoui.yassine@outlook.com', 'حي الزهور، فيلا 8'),
      ('STU-2026-0004', '1201198765', 'يوسف', 'قادري', 'Youssef', 'Kadri', 'MALE', '2011-11-05', 'الجزائر العاصمة', 'AB+', ?, ?, '2025-09-01', 'ACTIVE', '0554567890', 'kadri.rachid@gmail.com', 'نهج الحرية، رقم 102')
  ...
  const [seedStudents] = await pool.query(
    `SELECT id, matricule FROM students WHERE matricule IN ('STU-2026-0001', 'STU-2026-0002', 'STU-2026-0003', 'STU-2026-0004')`,
  );
  const studentIdByMatricule = Object.fromEntries(
    seedStudents.map((student) => [student.matricule, student.id]),
  );
  ...
  const sampleGuardians = [
    {
      ...
      nin: "1202016254001",
      phone: "0551234567",
      email: "halimi.mohamed@gmail.com",
      job: "موظف",
      is_primary: 1,
    },
    {
      matricule: "STU-2026-0002",
      relationship: "FATHER",
      name: "كمال بن سالم",
      nin: "1201725364002",
      phone: "0662345678",
      email: "bensalem.kamel@yahoo.fr",
      job: "تاجر",
      is_primary: 1,
    },
    {
      matricule: "STU-2026-0003",
      relationship: "MOTHER",
      name: "ياسين عمراوي",
      nin: "1201736452003",
      phone: "0773456789",
      email: "amraoui.yassine@outlook.com",
      job: "مهندس",
      is_primary: 1,
    },
    {
      matricule: "STU-2026-0004",
      relationship: "FATHER",
      name: "رشيد قادري",
      nin: "1201198765004",
      phone: "0554567890",
      email: "kadri.rachid@gmail.com",
      job: "مدير",
      is_primary: 1,
    },
  ];
  ...
  for (const guardian of sampleGuardians) {
    const studentId = studentIdByMatricule[guardian.matricule];
    if (!studentId) continue;

    await pool.query(
      `INSERT INTO student_guardians (student_id, relationship, name, nin, phone, email, job, is_primary)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const [feeRows] = await pool.query(
    `SELECT id, default_amount FROM fee_types WHERE frequency = 'MONTHLY' LIMIT 2`,
  );
  ...
  if (studentRows.length > 0 && feeRows.length > 0) {
    ...
    const [existingSale] = await pool.query(
      `SELECT id FROM product_sales WHERE invoice_number = 'POS-2026-0001'`,
    );
    let saleId;
    if (existingSale.length === 0) {
      ...
      const [pRows] = await pool.query(
        `SELECT id, selling_price FROM products LIMIT 2`,
      );
      if (pRows.length >= 2) {
        await pool.query(
          `
        ...
        `,
          [
            saleId,
            pRows[0].id,
            pRows[0].selling_price,
            pRows[0].selling_price,
            saleId,
            pRows[1].id,
            pRows[1].selling_price,
            pRows[1].selling_price,
          ],
        );
      }
