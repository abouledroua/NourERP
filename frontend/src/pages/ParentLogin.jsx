...
import { Phone, ArrowLeft, ArrowRight, ShieldCheck, Sparkles, AlertCircle, HeartHandshake } from 'lucide-react';
...
export default function ParentLogin() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    if (!phone || !phone.trim()) {
      setError(t('parent_portal.login_prompt', 'يرجى إدخال رقم الهاتف المسجل'));
      return;
    }
    ...
    try {
      ...
      const res = await api.post('/parent/login', { phone: phone.trim() });
      if (res.success && res.token) {
        localStorage.setItem('alnour_parent_token', res.token);
        localStorage.setItem('alnour_parent_info', JSON.stringify(res.parent));
        if (res.children && res.children.length > 0) {
          localStorage.setItem('alnour_parent_children', JSON.stringify(res.children));
        }
        navigate('/parent');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-navy-950 to-slate-900 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="absolute top-6 right-6 z-20 flex items-center bg-slate-800/80 backdrop-blur-md p-1 rounded-2xl border border-slate-700/80 text-xs font-bold text-white">
        <button
          onClick={() => setLang('ar')}
          className={`px-2.5 py-1 rounded-xl transition-all ${lang === 'ar' ? 'bg-emerald-600 text-white font-extrabold shadow-sm' : 'text-slate-400 hover:text-white'}`}
        >
          العربية
        </button>
        <button
          onClick={() => setLang('fr')}
          className={`px-2.5 py-1 rounded-xl transition-all ${lang === 'fr' ? 'bg-emerald-600 text-white font-extrabold shadow-sm' : 'text-slate-400 hover:text-white'}`}
        >
          Français
        </button>
        <button
          onClick={() => setLang('en')}
          className={`px-2.5 py-1 rounded-xl transition-all ${lang === 'en' ? 'bg-emerald-600 text-white font-extrabold shadow-sm' : 'text-slate-400 hover:text-white'}`}
        >
          English
        </button>
      </div>
        ...
        <div className="bg-slate-900/85 backdrop-blur-2xl border border-slate-800/90 rounded-3xl p-8 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
              {t('parent_portal.subtitle', 'متابعة مسار التمدرس، التخطيط الأسبوعي، والوضعية المالية لأبنائكم')}
            </p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-2">
                {t('parent_portal.phone_label', 'رقم الهاتف المسجل')}
              </label>
              <div className="relative">
                <input
                  ...
                  placeholder={t('parent_portal.phone_placeholder', '0550 12 34 56')}
              ...
              <p className="text-[11px] text-slate-500 mt-1.5 leading-normal">
                {t('parent_portal.login_prompt', 'أدخل رقم هاتفك المسجل لدى إدارة المدرسة للدخول المباشر')}
              </p>
            </div>
