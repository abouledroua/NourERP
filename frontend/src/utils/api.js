...
export function getWorkstationFingerprint() {
  let fp =
    localStorage.getItem("alnour_device_key") ||
    localStorage.getItem("alnour_workstation_fp");
...
export async function syncDeviceIdentity() {
  try {
    ...
    const res = await apiRequest(`/auth/device-identity?key=${currentKey}`, {
      method: "GET",
    });
...
export default {
  ...
  post: (url, body, options) =>
    apiRequest(url, { method: "POST", body, ...options }),
  put: (url, body, options) =>
    apiRequest(url, { method: "PUT", body, ...options }),
  delete: (url, options) => apiRequest(url, { method: "DELETE", ...options }),
  upload: (url, formData, options) =>
    apiRequest(url, { method: "POST", body: formData, ...options }),
};
