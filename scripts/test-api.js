...
// We can run test against http://127.0.0.1:5000
...
async function runTests() {
  ...
  try {
    ...
    console.log('       KPIs loaded! Tuition Revenue:', kpisRes.data.data.tuitionRevenue, 'Store Revenue:', kpisRes.data.data.storeRevenue, 'Net Cash:', kpisRes.data.data.netCash);

    // 9. Store POS Products & Invoices
    console.log('[9/12] Testing POS Products & Invoices (/api/pos/products, /api/pos/invoices)...');
    ...
    const invoicesRes = await request('GET', '/api/pos/invoices', null, authHeaders);
    if (invoicesRes.status !== 200 || !invoicesRes.data.success) {
      throw new Error(`POS invoices failed: ${JSON.stringify(invoicesRes.data)}`);
    }
    console.log('       POS Invoices loaded! Count:', invoicesRes.data.data.length);

    // 10. Timetable & Attendance
    console.log('[10/12] Testing Timetable (/api/timetable) & Attendance (/api/attendance)...');
    ...
    const attRes = await request('GET', '/api/attendance', null, authHeaders);
    if (attRes.status !== 200 || !attRes.data.success) {
      throw new Error(`Attendance endpoint failed: ${JSON.stringify(attRes.data)}`);
    }
    console.log('       Timetable slots:', ttRes.data.data.length, 'Attendance records:', attRes.data.data.length);

    // 11. Grades
    console.log('[11/12] Testing Grades (/api/grades)...');
    const gradesRes = await request('GET', '/api/grades', null, authHeaders);
    if (gradesRes.status !== 200 || !gradesRes.data.success) {
      throw new Error(`Grades endpoint failed: ${JSON.stringify(gradesRes.data)}`);
    }
    console.log('       Grades loaded! Count:', gradesRes.data.data.length);

    // 12. Audit Logs
    console.log('[12/12] Testing Audit Logs (/api/audit)...');
    ...
    console.log('       Audit Logs loaded! Count:', auditRes.data.data.length);
